      ...
      this.groupService
        ...
        .pipe(
          mergeMap((userGroups: UserGroupDTO[]) => {
            ...
            const filteredUsers = userGroups.filter((userGroup) => userGroup.keycloakId !== this.loggedInUserId);
        ...
        .subscribe(
          (usersWithDetails) => {
            this.usersInGroups[validGroupId] = usersWithDetails.filter(user => user.keycloakId !== this.loggedInUserId);
