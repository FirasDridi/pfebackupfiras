...
public class NotificationService {
    ...
    public void notifyAdminOfSubscriptionRequest(Long subscriptionRequestId, Long userId, Long groupId) {
        ...
        String serviceName = request.getServiceName();  // Fetch the service name

        Notification notification = new Notification();
        notification.setMessage(user.getUsername() + " wants to subscribe to " + (serviceName != null ? serviceName : "Unknown Service"));
        notification.setRead(false);
        notification.setTimestamp(LocalDateTime.now());
        notification.setUser(user);  // Ensure `user` includes username and firstname
    ...
    public List<Notification> getUnreadNotifications() {
        ...
        // Load user details for each notification to avoid null user
        notifications.forEach(notification -> {
            if (notification.getUser() == null && notification.getSubscriptionRequest() != null) {
                Long userId = notification.getSubscriptionRequest().getUserId();
                if (userId != null) {
                    User user = userRepository.findById(userId)
                            .orElseThrow(() -> new RuntimeException("User not found: " + userId));
                    notification.setUser(user);
